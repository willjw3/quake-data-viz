...
export const Chart = ({name, quakes}:{name: string, quakes: any}) => {
    ...
    for (let i = 0; i < 7; i++) {
        daysKey[i] = {date: new Date(Date.now() - 86400*1000*i).getDate(), count: 0, month: new Date(Date.now() - 86400*1000*i).getMonth() + 1}
    }

    quakes.forEach((quake: any) => {
        for (let i = 0; i < 7; i++) {
            if (new Date(quake.properties.time).getDate() === daysKey[i].date) {
               daysKey[i] = {date: daysKey[i].date, count: daysKey[i].count + 1, month: daysKey[i].month}; 
            }
        }
    })
    ...
    const drawChart = () => {
        ...
        const svg = d3.select(chartCanvas.current)
            .append("svg")
            .attr("width", '100%')
            .attr("height", '100%')
    ...
    return (
        <div className="chart">
            <h3>Last 7 Days by State: <span style={{color: 'gray'}}> {name}</span></h3>
            <div className="chart-wrapper" ref={chartCanvas}>    
            </div>
        </div>
    )
}