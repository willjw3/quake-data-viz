...
export const DataList = ({ frequency }:{frequency: string}) => {
    ...
    return (
        <div className="datalist-wrapper">
            <h3>The Latest: <span style={{color: 'gray'}}>{frequencyDisplay[frequency]}</span></h3>
            <div className="datalist">
                {
                    quakeData.length && quakeData.map((quake, i) => {
                    return (
                        <div key={i} className="quake-list-item">
                            ...
                            <p id="date"><strong>when: </strong>{new Date(quake.properties.time).toLocaleString()}</p>
                            <hr />
                        </div>
                    )
                    })
