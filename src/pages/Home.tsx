  ...
  const handleStateSelection = async (id: number) => {
    ...
    await quakeInfo.then(async (data: any) => {
      const quakes = await data.filter((quake: any) => {
        return quake.properties.place.includes(stateInfo.abbreviation) || quake.properties.place.includes(stateInfo.name)
      })
      setStateQuakes(quakes)
    })
  }
      ...
      <div className="app">
        ...
        <div className="button-case">
            ...
            <div className="input-group">
              <input className="radio" value="hour" type="radio" name="frequency" onClick={() => setFrequency('hour')} />
              <label className="frequency-setting" htmlFor="hour">hour</label>
              <input className="radio" value="day" type="radio" name="frequency" onClick={() => setFrequency('day')} defaultChecked/>
              <label className="frequency-setting" htmlFor="day">24 hours</label>
              <input className="radio" value="week" type="radio" name="frequency" onClick={() => setFrequency('week')} />
              <label className="frequency-setting" htmlFor="week">7 days</label>
            </div>
        </div>
